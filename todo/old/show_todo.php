...
$sth = mysql_query($stmt,$dbh);
$ft="";
while($row2 = mysql_fetch_array($sth, MYSQL_ASSOC)) {
  echo "<tr><td></td><td>";
  if($ft!="ft") {
    echo "Requires";
    $ft="ft";
  ...
  echo "</td><td>";
  $stmt = "SELECT * from todo where id=$row2[pre]";
  $sth1 = mysql_query($stmt,$dbh);
  $row1 = mysql_fetch_array($sth1, MYSQL_ASSOC);
  echo "<a href=show_todo.php?id=$row1[id]>$row1[note]</a></td></tr>";
}
...
<hr>
<a href=note_todo.php?id=<? echo $id; ?> >Notes</a>
<a href=edit_todo.php?id=<? echo $id; ?> >Edit</a>
